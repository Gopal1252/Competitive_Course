//https://www.spoj.com/problems/GSS3/   {Basically GSS1 with point updates}
#include<bits/stdc++.h>
using namespace std;
...
void updateNode(int i, int y, int ss, int se, int index){
    //out of bounds
    if(i<ss || se<i){
        return;
    }
    ...
    if(ss == se){
        ...
        tree[index].suffix = y;
        tree[index].total = y;
        tree[index].max_sum = y;
        return;
    }

    int mid = (ss+se)/2;
    updateNode(i,y,ss,mid,2*index);
    updateNode(i,y,mid+1,se,2*index+1);
